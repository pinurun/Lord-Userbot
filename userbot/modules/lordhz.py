...
async def _(hazmat):
    await hazmat.edit("`ᴘ ɪ ɴ ᴜ ʀ ᴜ ɴ Ingin Mengaktifkan Perintah Hazmat ヅ`")
    level = hazmat.pattern_match.group(2)
    if hazmat.fwd_from:
        return
    if not hazmat.reply_to_msg_id:
        await hazmat.edit("`Mohon Balas Ke Sticker/Gambar ヅ`")
        return
    ...
    async with hazmat.client.conversation(chat) as conv:
        try:
            ...
            await hazmat.reply("`Mohon Buka Blokir` @hazmat_suit_bot `Lalu Coba Lagi`")
            return
